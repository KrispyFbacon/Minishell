...
/*   Updated: 2025/05/13 18:51:34 by yes              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	errno = status;
	free(shell); // ft_kill without unlink
	rl_clear_history();
	exit(status);
}
...
void	child_exec_handler(t_shell *shell, t_node *node)
{
	...
	path = resolve_path(node->cmd, shell->envp);
	if (!path)
	{
		ft_printf(2, PATH_NOT_FOUND);
...
// TODO what to do
/* void	execute_process(t_shell *shell) <- general
void	execute_child(t_shell *shell) <- with pipes
void	execute_parent(t_shell *shell) <- no pipes */
